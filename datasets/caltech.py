...
class Caltech_Dataset(BaseDataset):
    ...
    def __init__(self, image_set, mode, eval) -> None:
        ...
        if eval == 'mr':
            if mode == 'train':
                self.targets,self._image_paths = self._remove_bad_images_new()
        else:
            self.targets,self._image_paths = self._remove_bad_images_new()
    ...
    def _load_image_paths_new(self) -> list:
        image_paths = []
        ann_dic = {"train": "anno_train_1xnew","val": "anno_test_1xnew"}
        images_root_path = os.path.join(self._image_set,'Caltech_new_annotations',ann_dic[self._mode])
        files = os.listdir(images_root_path)
        image_paths += [self._image_set + "/" + name[:-4].replace('_','/').replace('I','images/I') for name in files if
                        name.endswith('.txt')]
        return image_paths
    ...
    def _load_annotations_new(self) -> Tuple[list, list]:
        targets = []
        ann_dic = {"train": "anno_train_1xnew","val": "anno_test_1xnew"}
        ann_root_path = os.path.join(self._image_set,'Caltech_new_annotations',ann_dic[self._mode])
        files = os.listdir(ann_root_path)

        for file in files:
            target = {}
            id = torch.tensor(int(file[3:5] + file[7:10] + file[12:17]))
            ann_path = os.path.join(ann_root_path,file)
            with open(ann_path,"r") as txt_file:
                data = txt_file.read().split('\n')[:-1]
                if len(data) > 1:
                    boxes = []
                    for tar in data[1:]:
                        if tar.split()[0] == 'person': # and float(tar.split()[4])>50:
                            boxes.append(list(map(float,tar[7:].split()))[:4])
                    box = torch.as_tensor(boxes, dtype=torch.float32).reshape(-1, 4)
                    target['boxes'] = box
                    target['boxes'][:,2] = box[:,0] + box[:,2]
                    target['boxes'][:,3] = box[:,1] + box[:,3]
                    target['image_id'] = id
                    target['labels'] = torch.ones(len(box),dtype=int)
        ...
        for index, single_id in enumerate(self.image_ids):
            ...
            with open(ann_path) as json_file:
                ...
                if len(data) > 0:
                    ...
                    for tar in data:
                        num_person = 0
                        if tar["lbl"] == "person": #and tar["pos"][2]>20 and tar["pos"][3]>50:
                            num_person += 1
    ...
    def _remove_bad_images_new(self):
        targets,image_paths = [],[]
        for target,image_path in zip(self.targets,self._image_paths):
            if target['boxes'] is not None:
                targets.append(target)
                image_paths.append(image_path)
                # image_ids.append(image_id)
